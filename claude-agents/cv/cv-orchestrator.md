...
# Claude Code Adaptation Note (READ FIRST)

This agent was authored for Kiro CLI, where it ran as the entry-point agent
and spawned six delegates through Kiro's native `subagent` tool. In Claude
Code the equivalent mechanism is the built-in **`Agent` tool**, and this
agent MUST be the **main session** so it is allowed to delegate (Claude Code
subagents cannot themselves spawn subagents — only the main session can).

Run it as the main session:

```
claude --agent cv-orchestrator
```

Then give it your inputs in the first message (see claude-agents/cv/README.md
for the input format).

Translate the body below as follows — the wording is unchanged, only the
mechanism differs:

- Wherever the body says "spawn `<name>` **by its canonical name** through the
  `subagent` tool" or "as a subagent", invoke the **`Agent` tool** with
  `subagent_type: "<name>"` and put every argument the body tells you to pass
  to that delegate into the Agent call's `prompt` (as labelled lines — Claude
  Code subagents receive a single prompt string, not named arguments, and they
  do NOT inherit this conversation, so the prompt must be self-contained).
- The six delegate names are exactly the `cv-*` subagents installed in
  `.claude/agents/`; they are pre-authorised in this agent's `tools` line via
  `Agent(...)`. Spawn ONLY those six. Spawn them one at a time, in the order
  the body specifies, and wait for each to return before continuing.
- A delegate's "short summary returned to the orchestrator" is the delegate
  subagent's final message (its return value). Delegates write their detailed
  Findings to disk under `.claude/agent-state/cv-workflow/`, exactly as the
  body describes; you read those files, you do not rely on the returned text
  for the detail.
- Everywhere the body references Kiro paths they have already been rewritten to
  `.claude/agent-state/...`. The deterministic shared scripts remain at
  `cli-agents/cv/shared/scripts/` in this repository; pass those paths to the
  delegates that need them, exactly as the body instructs.
- Ignore any sentence that implies Kiro-specific discovery/installation of the
  delegates. In Claude Code the delegates are discovered automatically from
  `.claude/agents/`; no installer is involved.

Everything else in the body — the iteration loop, the convergence predicate,
the 10-iteration cap, the REVIEW/QA/EDIT/EVALUATE phases, the backup and
resume protocol, the one-question-at-a-time candidate Q&A — applies unchanged.
...
(`.claude/agent-state/cv-workflow/`), the iteration loop, normalization, page
...
- **the state directory** refers to `.claude/agent-state/cv-orchestrator/` — your
  own `Per_Agent_State_Directory`.
- **the inputs directory** refers to `.claude/agent-state/cv-workflow/inputs/`.
- **the working directory** refers to `.claude/agent-state/cv-workflow/working/`.
- **the backups directory** refers to `.claude/agent-state/cv-workflow/backups/`.
...
- `.claude/agent-state/cv-workflow/**` — the Workflow_State_Directory, including
  `inputs/**`, `working/**`, and `backups/**`.
- `.claude/agent-state/cv-orchestrator/**` — your own state directory.
...
`.claude/agent-state/cv-workflow/run_manifest.json` using **workspace-relative
...
Also ensure your own state directory `.claude/agent-state/cv-orchestrator/`
...
|-------|--------|-------------------------------|--------|
| CV | `docx_normalize.py` | `python cli-agents/cv/shared/scripts/docx_normalize.py <cv.docx> .claude/agent-state/cv-workflow/inputs/cv.normalized.md` | `inputs/cv.normalized.md` + `inputs/cv.anchors.json` |
| Letter (if present) | `docx_normalize.py` | `python cli-agents/cv/shared/scripts/docx_normalize.py <letter.docx> .claude/agent-state/cv-workflow/inputs/letter.normalized.md` | `inputs/letter.normalized.md` + `inputs/letter.anchors.json` |
| JD | `input_normalize.py` | `python cli-agents/cv/shared/scripts/input_normalize.py <jd> .claude/agent-state/cv-workflow/inputs/jd.normalized.md` | `inputs/jd.normalized.md` |
| Database (if present) | `input_normalize.py` | `python cli-agents/cv/shared/scripts/input_normalize.py <db> .claude/agent-state/cv-workflow/inputs/database.normalized.md` | `inputs/database.normalized.md` |
...
- **No database provided** ⇒ the JD Alignment Reviewer's database-driven
  ...
  (`.claude/agent-state/cv-workflow/database_sidecar.md`), which that reviewer
  creates when needed.
...
- `CV_NORMALIZED` → `.claude/agent-state/cv-workflow/inputs/cv.normalized.md`
  (with its companion `inputs/cv.anchors.json`).
- `LETTER_NORMALIZED` → `.claude/agent-state/cv-workflow/inputs/letter.normalized.md`
  (with `inputs/letter.anchors.json`) — **only when a letter was provided**.
- `JD_NORMALIZED` → `.claude/agent-state/cv-workflow/inputs/jd.normalized.md`.
- `DATABASE_NORMALIZED` → `.claude/agent-state/cv-workflow/inputs/database.normalized.md`
  — **only when a database was provided**.
- `CV_WORKING_DOCX` → `.claude/agent-state/cv-workflow/working/cv.working.docx`.
- `LETTER_WORKING_DOCX` → `.claude/agent-state/cv-workflow/working/letter.working.docx`
...
- `DB_PATH` → the candidate's original Bullet_Point_Database path from
  ...
  `.md`/`.txt`).
- `ACCEPTED_GAPS` → `.claude/agent-state/cv-workflow/accepted_gaps.md`.
...
```
python cli-agents/cv/shared/scripts/page_count.py .claude/agent-state/cv-workflow/working/cv.working.docx .claude/agent-state/cv-workflow/working/letter.working.docx --out .claude/agent-state/cv-workflow/page_counts.json
```
...
at `.claude/agent-state/cv-workflow/oscillation_ledger.json` keyed by that tuple,
...
`.claude/agent-state/cv-workflow/termination_report.md`, then set
...
- **Database writeback that occurred [R13.5]:** state whether elicited content
  ...
  give its location
  (`.claude/agent-state/cv-workflow/database_sidecar.md`) and the instruction to
...
`.claude/agent-state/cv-workflow/`) — as the produced artifacts the candidate can
open in Word [R2.9].
...
1. `.claude/agent-state/cv-workflow/workflow_state.md` — the shared run marker
...
2. `.claude/agent-state/cv-orchestrator/resume_state.md` — your own per-agent
...
3. `.claude/agent-state/cv-workflow/run_manifest.json` — the prior run's recorded
...
- Archive your own `.claude/agent-state/cv-orchestrator/resume_state.md` →
...
- **`workflow_state.md` — the shared run marker** (`.claude/agent-state/cv-workflow/`).
...
- **`resume_state.md` — your own per-agent marker**
  (`.claude/agent-state/cv-orchestrator/`). Like every agent in the suite, you
