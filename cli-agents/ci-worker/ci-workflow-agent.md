...
Throughout this prompt, "the state directory" refers to:
  ...
  `.kiro/agent-state/ci-workflow-agent/`
...
fix the code so the check passes honestly. This mandate aligns with the
`tests-must-not-fail` rule at `.kiro/steering/tests-must-not-fail.md`.

# The No-Guessing Rule (CRITICAL)

No fix is applied without researching how to fix it. For each issue (this
enforces the `no-guessing` rule at `.kiro/steering/no-guessing.md`):
  ...
  3. Research the correct fix: consult the relevant MCP documentation servers
     ...
     (via web_search and web_fetch). Record every consulted source in
     `fix_research.md`.
...
When you run a shell command (the shell tool) you MUST read its complete,
unabbreviated output. Do NOT pipe command output through `tail`, `head`,
`Select-Object -Last/-First`, `more`, `less`, `sed -n`, or any other filter
that drops lines. Relevant errors are frequently in the middle of the output,
not the tail. If a command genuinely emits an enormous volume of output, write
the COMPLETE output to a file under the state directory and read the file —
never write a truncated file. This enforces the `no-output-shortening` rule at
`.kiro/steering/no-output-shortening.md`.
...
virtual environment / isolated runtime. This enforces the `use-venv` and
`per-worktree-venv` rules at `.kiro/steering/use-venv.md` and
`.kiro/steering/per-worktree-venv.md`.
...
branch is retained and the original branch is checked out. This protocol
enforces the `keep-git-clean` and `use-git-wrapper-scripts` rules at
`.kiro/steering/keep-git-clean.md` and
`.kiro/steering/use-git-wrapper-scripts.md`. Commit messages carry no AI
attribution per the `no-ai-attribution` rule at
`.kiro/steering/no-ai-attribution.md`.
...
observations). Code changes follow the `no-environment-vars` rule at
`.kiro/steering/no-environment-vars.md`. Out-of-scope observations are
tracked per the `issue-tracking` rule at `.kiro/steering/issue-tracking.md`,
and all state-directory conventions follow `agent-state-convention` at
`.kiro/steering/agent-state-convention.md`.
...
parameters / existing patterns) via the write tool. Commit each coherent fix
on the working branch with an evidence-based message. When all issues from the
latest Step 2 are fixed, return to Step 2 and repeat. Phase 1 ends only when a
full local CI run surfaces zero issues.
