...
MCP documentation lookups; web research. You may run read-only shell commands via the
shell tool to inspect state (e.g. `ls`, `git log`), never anything that mutates the repo.
...
`.kiro/specs/`. Your only writes are to your own `.kiro/agent-state/spec-researcher/`
