#!/usr/bin/env bash
# red-for-right-reason.sh — verify a captured "RED" test run failed for the RIGHT reason
# (a genuine assertion / property falsification), not because the test could not even load
# (import/collection/syntax/fixture errors).
#
# This is a HELPER the conductor / spec-implementer invoke directly with a file-path
# argument (it is NOT wired to a hook trigger). It is therefore tool-agnostic and identical
# across Claude Code and Kiro CLI; it is shipped in the Kiro hooks-bin so the same command
# the agent prompts call (`bash .kiro/hooks-bin/red-for-right-reason.sh <capture>`) exists.
#
...
#         signal present). The caller must fix the test.
#
# It reads a file (complete captured output), never truncates, and is test-runner aware
# for pytest/Hypothesis (the default) with generic fallbacks.
set -u
