...
**Two orchestrator-owned mechanics the hook does NOT do for you:**
  ...
  - **On a host without `jq` the hook records NO ENTRY AT ALL** (the steering file
    ...
    `environment.md` and in your final report (operator-fixable — install `jq`, or port
    the hook), put your state in the FIXED directory `runs/unregistered/` and use it
...
prose note is read by no hook — and surface a single clarity-first message. Then continue
with other workable issues if any remain (do not idle).
